...
using SDORP.Coverage;
using SDORP.Properties;
using System.Windows.Media;
using System.Windows.Threading;
using SDORP.Forwarding;
using SDORP.ExpermentsResults.Energy_consumptions;
using SDORP.ExpermentsResults.Lifetime;
using SDORP.ControlPlane.NOS.TC;
using SDORP.ControlPlane.NOS.TC.subgrapgh;
using SDORP.DataPlane.NeighborsDiscovery;
using SDORP.ControlPlane.NOS.FlowEngin;
using SDORP.ui.conts;
using SDORP.Charts.Intilization;
using SDORP.ControlPlane.NOS.Visualizating;
    ...
    public partial class MainWindow : Window
    {
        ...
        private void TimerCounter_Tick(object sender, EventArgs e)
        {
            ...
            if (PublicParamerters.SimulationTime <= stopSimlationWhen + PublicParamerters.MacStartUp)
            {
                ...
                Dispatcher.Invoke(() => Title = "SDORP:" + PublicParamerters.SimulationTime.ToString(), DispatcherPriority.Send);
            }
            else
            {
                TimerCounter.Stop();
        ...
        public void DisplaySimulationParameters(int rootNodeId, string deblpaymentMethod)
        {
            ...
            //lbl_par_Dir.Content = Settings.Default.ExpoRCnt;
            lbl_par_L.Content = Settings.Default.ExpoLCnt;
            lbl_par_R.Content = Settings.Default.ExpoECnt;
        ...
        public void RandomDeplayment(int sinkIndex)
        {
            ...
            string PowersString = "γL=" + Settings.Default.ExpoLCnt + ", γE=" + Settings.Default.ExpoECnt + ", γH" + Settings.Default.ExpoHCnt;
            ...
            //PublicParamerters.MyNetwork = myNetWork;
            HopsToSinkComputation.ComputeHopsToSink(PublicParamerters.SinkNode);

            //foreach (Sensor sen in myNetWork) { UplinkRouting.compute(sen); }

            ORPComputing(sinkIndex);
        ...
        public void ORPComputing(int sinkIndex)
        {
            int rootNodeId = sinkIndex;
            PublicParamerters.SinkNode = myNetWork[rootNodeId];

            //Sink sn = new Sink(myNetWork);
            //foreach (Sensor sen in PublicParamerters.MyNetwork)
            //    sn.ComputeORP();
            UplinkRouting orp = new UplinkRouting(myNetWork);
            //foreach (Sensor sen in PublicParamerters.MyNetwork)
            orp.compute();
                ...
                case "_Show Dead Node":
                    {
                        if (myNetWork.Count > 0)
                        {
                            if (PublicParamerters.DeadNodeList.Count > 0)
                            {
                                ...
                                xx.Title = "SDNORP Lifetime report";
